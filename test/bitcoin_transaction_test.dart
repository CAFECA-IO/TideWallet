...
void main() {
  ...
  setUpAll(() async {
    ...
    final _currencies = currencies
        .map(
          (c) => AccountCurrencyEntity(
              accountcurrencyId:
                  c['account_id'] ?? c['account_token_id'], // TODO: Change name
    ...
    await DBOperator().accountCurrencyDao.insertCurrencies(_currencies);
  });
  ...
  group('Bitcoin Transaction test', () {
    ...
    test('insertUtxo test', () async {
      List<JoinUtxo> utxos =
          await opt.utxoDao.findAllJoinedUtxosById('currencyId');
      Log.warning('utxos: ${hex.encode(Uint8List(0))}');
